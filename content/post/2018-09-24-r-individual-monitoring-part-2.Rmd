...
header:
  caption: ''
  image: headers/dashboard-snip.png
summary: Modeling and anomaly detection in R
bibliography: 3-Individual_Monitoring/3-refs.bib
csl: ijsm.csl
link-citations: yes
output:
  blogdown::html_page:
...
![](/post/3-Individual_Monitoring/alive.png){ width=80% height=80%}
...
![](/post/3-Individual_Monitoring/time-anomaly.png)
...
![](/post/3-Individual_Monitoring/both-worlds.png){width=60% height=60%}
...
As always, it's much easier to show than tell with this stuff, so how about an example. You can find the data for this post [here](/post/3-Individual_Monitoring/prediction-data.csv). 

```{r, echo=FALSE}
rpe.data <- read.csv('3-Individual_Monitoring/prediction-data.csv')
...
knitr::kable(rpe.data[1:10,]) %>% kable_styling(c("responsive","condensed"))
```
...
![](/post/3-Individual_Monitoring/gps-output.png)
